...
const logout = async function(){
		try {
		const res = await axios.post(`${BASE}/v1/api/auth/logout/`, {headers: {'Content-Type': 'application/json', Authorization: `Bearer ${state.data.user.access_token}`}})//config)
			const resp = res.data 
...
const login = async function(email, password) {
		try {
			...
			location.href=location.href.split("?redirect=")[1] || '/dashboard'
		} catch(errors) {
		alertify.alert(errors)
				}
	};
